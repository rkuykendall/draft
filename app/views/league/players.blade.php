					...
					<ul class="unstyled">
						@foreach($player->movies as $movie)
							<li>
								...
								<p>Current earnings: {{{ '$'.number_format($movie->latestEarnings->domestic) }}} - Bought for {{{ $league->units.' '.$movie->league_pivot->price }}} - {{{ '$'.number_format(round($movie->latestEarnings->domestic/($movie->league_pivot->price ?: 1), 2)).' per '.$league->units }}}</p>
							</li>
						@endforeach
					</ul>
				@else
