...
class UpdateEarnings {
	...
	public function fire($job, $data) {
		...
		$earnings = $movie->earnings()->where('date', Carbon::now("America/Los_Angeles")->format("Y-m-d"))->first();

		if(!$earnings) {
			$earnings = new MovieEarning(array("movie_id" => $movie->id, "date" => Carbon::now("America/Los_Angeles")));
		}
